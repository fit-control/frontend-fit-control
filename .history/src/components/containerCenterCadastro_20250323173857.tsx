...
const ContainerCenterCadastro = () => {
  ...
  const handleFormSubmit = async ({ nomeCompleto, emailCadastro, password, genero }: User) => {
    console.log("ENTROU AQUI", nomeCompleto, emailCadastro, password, genero);
    setLoading(true);

    try {
      const response = await login(emailCadastro, password)
      if (response) {
