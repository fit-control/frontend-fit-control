...
const ContainerCenterLogin = () => {
  ...
  // Função para exibir o toast com a mensagem de erro
  useEffect(() => {
    if (errors.email) toast.error(errors.email.type === "required" ? "Email is Required" : errors.email.message);
    if (errors.password) toast.error(errors.password.type === "required" ? "Password is Required" : errors.password.message);
